import{s}from"./supabase-CkHjHRYH.js";document.addEventListener("DOMContentLoaded",async()=>{const t=document.getElementById("ofertas-container"),a=document.getElementById("loading-ofertas");if(!(!t||!a))try{const{data:e,error:r}=await s.from("nuevaOferta").select("*").eq("estado","pendiente").order("id",{ascending:!1});if(r)throw r;if(a.style.display="none",!e||e.length===0){t.innerHTML=`
            ...
            `;return}let i="";e.forEach(n=>{i+=`
                <div class="card-premium" style="display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <a href="oferta.html?id=${n.id}" class="btn btn-primary w-100" style="padding: 10px; font-size: 0.9rem;">
                        Gestionar pedido
                    </a>
                </div>
            `}),t.innerHTML=i}catch(e){console.error("Error cargando ofertas:",e),a.style.display="none",t.innerHTML=`
            <div class="col-12 text-center mt-5">
