...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let existingFotoUrl: string | null = null;
    let existingAntecedentesUrl: string | null = null;
    let existingMatriculaUrl: string | null = null;
    ...
    async function loadInitialData() {
        ...
        if (perfil.nombre) (document.getElementById('reg-nombre') as HTMLInputElement).value = perfil.nombre;
        if (perfil.dni) (document.getElementById('reg-dni') as HTMLInputElement).value = perfil.dni;
        if (perfil.edad) (document.getElementById('reg-edad') as HTMLInputElement).value = perfil.edad;
        if (perfil.telefono) (document.getElementById('reg-telefono') as HTMLInputElement).value = perfil.telefono;
        if (perfil.zona_frecuente) cityInput.value = perfil.zona_frecuente;
        if (perfil.latitud) latInput.value = perfil.latitud;
        if (perfil.longitud) lonInput.value = perfil.longitud;
        if (perfil.antiguedad) (document.getElementById('reg-antiguedad') as HTMLInputElement).value = perfil.antiguedad;
        if (perfil.antecedentes) (document.getElementById('reg-antecedentes') as HTMLTextAreaElement).value = perfil.antecedentes;

        if (perfil.foto_url) {
            existingFotoUrl = perfil.foto_url;
            let publicUrl = existingFotoUrl;
            ...
            if (fotoPreview && publicUrl) fotoPreview.src = publicUrl;
        }

        if (perfil.oficios && perfil.oficios.length > 0) {
            const primerOficio = Array.isArray(perfil.oficios) ? perfil.oficios[0] : perfil.oficios;
            if (searchProfesion) searchProfesion.value = primerOficio;
            if (inputProfesionHidden) inputProfesionHidden.value = primerOficio;
        }

        if (perfil.antecedentes_url) existingAntecedentesUrl = perfil.antecedentes_url;
        if (perfil.matricula_url) existingMatriculaUrl = perfil.matricula_url;
    }
    ...
    searchProfesion?.addEventListener('input', () => {
        ...
        // Reset valid hidden input if typed something new entirely (forcing them to pick from list or lose valid data)
    ...
    btnVerTodas?.addEventListener('click', async (e) => {
        e.stopPropagation();
        const rect = btnVerTodas.getBoundingClientRect();
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        if (!selectedFoto && !existingFotoUrl) {
        ...
        try {
            ...
            if (selectedFoto) {
                fotoUrl = await uploadFile('avatars', '', selectedFoto, dni);
            }

            let docAntecedentesUrl = existingAntecedentesUrl;
            let docMatriculaUrl = existingMatriculaUrl;
            ...
            // Fetch User Auth from active session
            const { data: { session }, error: sessionError } = await supabase.auth.getSession();
            if (sessionError || !session) {
                window.location.href = '/login.html';
                throw new Error("No hay una sesión activa. Por favor, iniciá sesión primero.");
            }
